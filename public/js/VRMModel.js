class VRMModel {
  ...
  get nowEmotionValence() {
    return this.valenceArousal.v < 0 ? THREE.VRMSchema.BlendShapePresetName.Sorrow : THREE.VRMSchema.BlendShapePresetName.Fun;
  }
  get nowEmotionArousal() {
    return this.valenceArousal.a < 0 ? THREE.VRMSchema.BlendShapePresetName.Sorrow : THREE.VRMSchema.BlendShapePresetName.Fun;
  }

  // 表情のウェイト計算（不使用）
  get speakEmotionValenceValue () {
    // let weight = (this.clock.elapsedTime - this.startEmotionAnimateTime) / this.emotionAnimateTime;
    // let weight = 1;
    let weight = Math.abs(this.valenceArousal.v);
    console.log(weight);

    return 1 < weight ? 1 : weight;
  }
  get speakEmotionArousalValue () {
    // let weight = (this.clock.elapsedTime - this.startEmotionAnimateTime) / this.emotionAnimateTime;
    // let weight = 1;
    let weight = 0;
    if (this.valenceArousal.a < 0.5) {
      weight = (0.5 - this.valenceArousal.a) / 0.5;
    } else {
      weight = (this.valenceArousal.a - 0.5) / 0.5;
    }
    console.log(weight);
  ...
  // 表情値を設定する
  set setValenceArousal(vaObj) {
    this.valenceArousal = vaObj;
    console.log("Valence - Arousal :" + this.valenceArousal.v + "," + this.valenceArousal.a);
  }
  ...
  onAnimate() {
    ...
    if (this.model) {
      ...
      // this.model.blendShapeProxy.setValue(this.nowEmotionValence, this.speakEmotionValenceValue);
      // this.model.blendShapeProxy.setValue(this.nowEmotionArousal, this.speakEmotionArousalValue);
